      ...
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <ClientThemeToggle label="Toggle theme" />
            <Sheet>
              <SheetTrigger asChild className="md:hidden"><Button variant="outline" size="icon" aria-label="Menu"><Menu className="h-4 w-4" /></Button></SheetTrigger>
...
export async function GuideSidebar({ locale, navGroups, currentPath }: { locale: string; navGroups: NavGroup[]; currentPath?: string }) {
  const t = await getTranslations({ locale, namespace: "shared" });
  const isActive = (href: string) => currentPath === href;
  const navigationIconBySlug = new Map(NAVIGATION_CONFIG.map((item) => [item.key, item.icon]));
  const keyGuides = [
    ["What is Jev AI?", "/guide/jev-ai-explained"],
    ["Beginner Guide", "/guide/jev-ai-beginner-guide"],
    ["API Quickstart", "/api/jev-ai-quickstart"],
    ["Jev vs ChatGPT", "/comparisons/jev-ai-vs-chatgpt"],
  ];
  return <aside className="space-y-6 lg:sticky lg:top-24 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto lg:pr-1"><section className="rounded-2xl border border-border bg-card/60 p-5 shadow-sm"><h3 className="mb-4 text-xs font-bold uppercase tracking-[0.22em] text-muted-foreground">{t("guideNavigation")}</h3><div className="space-y-4">{navGroups.map((group) => { const Icon = navigationIconBySlug.get(group.slug); return <CollapsibleNavGroup key={group.slug} title={group.title} icon={Icon ? <Icon className="h-4 w-4 text-[hsl(var(--nav-theme))]" /> : <span className="grid h-4 w-4 place-items-center rounded text-[10px] font-bold text-[hsl(var(--nav-theme))]">{group.title[0]}</span>} count={group.count} currentPath={currentPath}><ul className="space-y-1">{group.links.map((link) => <li key={link.href}><Link href={localizeHref(link.href, locale)} className={`flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm transition-colors ${isActive(link.href) ? "bg-[hsl(var(--nav-theme)/0.15)] font-semibold text-[hsl(var(--nav-theme))]" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}><span className="truncate">{link.label}</span>{link.badge && <Badge variant="secondary" className="ml-auto h-5 border-border px-1.5 text-[10px]">{link.badge}</Badge>}</Link></li>)}</ul></CollapsibleNavGroup>; })}</div></section><section className="rounded-2xl border border-border bg-card/60 p-5"><h3 className="mb-3 text-sm font-bold text-foreground">{t("sidebarTitle")}</h3><div className="space-y-2 text-sm">{keyGuides.map(([label, href]) => <Link key={href} href={localizeHref(href, locale)} className="block rounded-xl bg-muted p-3 font-medium text-foreground transition hover:text-[hsl(var(--nav-theme))]">{label}</Link>)}<Link href={localizeHref("/guide", locale)} className="inline-flex items-center gap-1 text-sm font-semibold text-[hsl(var(--nav-theme))]">{t("viewAllGuides")} <ChevronRight className="h-4 w-4" /></Link></div></section><section className="rounded-2xl border border-border bg-card/60 p-5"><h3 className="mb-3 text-sm font-bold text-foreground">Promo Codes</h3><div className="space-y-2"><div className="rounded-xl bg-muted p-3"><code className="text-sm font-bold text-foreground">No codes available</code><p className="mt-1 text-xs leading-5 text-muted-foreground">Jev AI is an API model, not a game with redeem codes.</p></div><div className="rounded-xl bg-muted p-3"><code className="text-sm font-bold text-foreground">No codes available</code><p className="mt-1 text-xs leading-5 text-muted-foreground">Use official TypeSafe or provider pages for access details.</p></div></div></section></aside>;
}

export async function SiteFooter({ locale }: { locale: string }) {
  const t = await getTranslations({ locale, namespace: "footer" });
  const site = await getTranslations({ locale, namespace: "site" });
  return <footer className="mt-16 border-t border-border bg-card/30"><div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8"><div className="mb-10 rounded-2xl border border-border bg-muted/40 p-5"><div className="font-bold text-foreground">{site("name")}</div><p className="mt-1 text-sm text-muted-foreground">{t("description")}</p><Link href="https://typesafe.ai" className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-[hsl(var(--nav-theme))]">TypeSafe AI <ExternalLink className="h-4 w-4" /></Link></div><p className="mb-8 text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">{site("legalNotice")}</p><div className="grid gap-8 md:grid-cols-4"><div className="md:col-span-2"><h3 className="font-bold text-foreground">{t("aboutTitle")}</h3><p className="mt-3 max-w-xl text-sm leading-7 text-muted-foreground">{t("about")}</p></div><FooterList title={t("quickLinks")} links={[[t("playGame"), "https://typesafe.ai"], [t("officialDiscord"), "https://docs.typesafe.ai/introduction.md"], [t("officialYoutube"), "https://typesafe.ai/blog/introducing-system-one-models-and-jev"], [t("cloudflareModel"), "https://developers.cloudflare.com/ai/models/typesafe/jev/"]]} /><FooterList title={t("guides")} links={[[t("beginnerGuide"), "/guide/jev-ai-beginner-guide"], [t("modelGuides"), "/model"], [t("apiGuides"), "/api"], [t("integrationGuides"), "/integrations"], [t("privacyPolicy"), "/privacy-policy"], [t("termsOfService"), "/terms-of-service"]]} /></div><p className="mt-10 border-t border-border pt-6 text-xs text-muted-foreground">{t("copyright")}</p></div></footer>;
}
  ...
  return (
    <div className="group relative cursor-pointer overflow-hidden rounded-2xl border border-border shadow-lg transition-all duration-200">
      <div className="relative aspect-video w-full">
        <img src="/images/hero-trailer-thumbnail.jpg" alt="Jev AI overview video" className="size-full object-cover transition-all duration-200 group-hover:brightness-80" />
      </div>
