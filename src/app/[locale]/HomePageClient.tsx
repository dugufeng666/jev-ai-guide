...
import { ArrowRight, Bot, BookOpen, Boxes, BrainCircuit, ChevronRight, CircleHelp, Code2, Compass, GitCompare, Layers3, Network, ScrollText, Shield, Workflow, Zap, type LucideIcon } from "lucide-react";
...
const icons: LucideIcon[] = [BookOpen, Bot, Code2, BrainCircuit, Workflow, Network, GitCompare, Layers3, Shield, Compass, Boxes, Zap, CircleHelp, ScrollText];
...
export default function HomePageClient({ home, locale, articles, recentArticles }: { home: Home; locale: string; articles: ContentItem[]; recentArticles: ContentItem[] }) {
  const YOUTUBE_VIDEO_ID = "QbYBRjOaGOo";
            ...
            {home.explore.modules.map((mod) => (
              <div key={mod.order} className="overflow-hidden rounded-2xl border border-border bg-card/70">
                ...
                <div className="px-6 py-5">
                  {/* code-cards: compact highlighted resources */}
                      ...
                      {mod.highlights.map((h, i) => {
                        ...
                        return (
                          <div key={i} className={`rounded-xl border p-4 ${tierColor}`}>
                            <span className={`text-sm font-bold ${tierText}`}>{h.label}</span>
