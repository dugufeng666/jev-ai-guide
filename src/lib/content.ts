 ...
 * Example: slug="jev-ai-api" maps back to the matching MDX filename
 */
...
/**
 * 导航分组结构（用于动态 Guide Navigation）
 */
export interface NavGroup {
  /** Group title, from directory slug to human-readable label. */
  title: string;
  /** 该分组下的文章数量 */
  count: number;
  /** Group slug, matching the content directory name. */
  slug: string;
  /** 文章链接列表 */
...
const GROUP_ORDER: string[] = [
  "guide", "model", "api", "typesafe", "agent", "integrations", "comparisons", "community",
];

/**
 * 动态生成 Guide Navigation 分组
